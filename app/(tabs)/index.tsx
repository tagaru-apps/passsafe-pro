...
export default function HomeScreen() {
  ...
  const { t, certTrack, readiness, progress, usage, language, examDate, unlockRewarded } = usePassSafe();
  ...
  const refreshRewardAvailability = useCallback(async () => {
    ...
    const availability = await checkRewardedAdAvailability();
    setRewardAvailability(availability);
  }, []);
  ...
  const watchReward = async () => {
    ...
    if (completed) {
      ...
      haptic.error();
      setRewardFeedback("A reward is not available right now. Upgrade to Pro for unlimited questions.");
      setRewardAvailability("unavailable");
    }
    setIsUnlocking(false);
    if (completed) void refreshRewardAvailability();
  };
  return (
    <ScreenContainer style={styles.screen}>
      <ScrollView contentContainerStyle={[styles.content, isCompact && styles.contentCompact]} showsVerticalScrollIndicator={false}>
        ...
        {usage.isPro ? <View style={styles.proAllowance}><MaterialIcons name="verified" size={18} color="#FFFFFF" /><View style={styles.proAllowanceCopy}><Text style={styles.proAllowanceTitle}>PassSafe Pro</Text><Text style={styles.proAllowanceText}>Unlimited questions today · No daily limit</Text></View></View> : questionProgress ? <Card style={[styles.allowanceCard, isCompact && styles.cardCompact]}><View style={styles.allowanceHeader}><View><Text style={styles.allowanceTitle}>Daily free questions</Text><Text style={styles.allowanceSubtext}>{questionProgress.used} of {questionProgress.allowance} questions used today</Text></View><View style={styles.remainingBadge}><Text style={styles.remainingValue}>{questionProgress.remaining}</Text><Text style={styles.remainingLabel}>left</Text></View></View><View style={styles.allowanceProgress}><ProgressBar value={questionProgress.percentage} color={questionProgress.remaining <= 10 ? colors.accent : colors.primary} /></View><Pressable accessibilityRole="button" accessibilityLabel={rewardAvailability === "unavailable" ? "Upgrade to Pro for unlimited questions" : "Watch rewarded ad to unlock 10 more questions"} disabled={isUnlocking || rewardAvailability === "checking"} onPress={watchReward} style={({ pressed }) => [styles.rewardCta, rewardAvailability === "unavailable" && styles.rewardUpgradeCta, (isUnlocking || rewardAvailability === "checking") && styles.rewardCtaDisabled, pressed && styles.pressed]}>{isUnlocking || rewardAvailability === "checking" ? <ActivityIndicator size="small" color="#FFFFFF" /> : <MaterialIcons name={rewardAvailability === "unavailable" ? "workspace-premium" : "play-circle-filled"} size={22} color="#FFFFFF" />}<View style={styles.rewardCtaCopy}><Text style={styles.rewardCtaTitle}>{isUnlocking ? "Loading your reward…" : rewardAvailability === "checking" ? "Checking reward availability…" : rewardAvailability === "unavailable" ? "Reward unavailable · Go Pro" : "Watch ad · +10 questions"}</Text><Text style={styles.rewardCtaSubtitle}>{isUnlocking ? "Keep this screen open" : rewardAvailability === "unavailable" ? "Unlimited questions, no ads" : "Reward ready · unlock instantly after completion"}</Text></View><MaterialIcons name="chevron-right" size={21} color="#FFFFFF" /></Pressable>{rewardFeedback ? <Text style={[styles.rewardFeedback, rewardFeedback.startsWith("10") ? styles.rewardSuccess : styles.rewardFailure]}>{rewardFeedback}</Text> : null}<Text style={styles.allowanceHint}>{usage.rewardedUnlocks > 0 ? `Your ${usage.rewardedUnlocks * 10} rewarded bonus questions are included.` : "No subscription needed. You can watch a reward whenever you want more practice."}</Text></Card> : null}
