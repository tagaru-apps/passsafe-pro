...
export default function HomeScreen() {
  ...
  useEffect(() => {
    Animated.timing(allowanceProgress, { toValue: displayedRemainingProgress / 100, duration: 420, useNativeDriver: false }).start();
  }, [allowanceProgress, displayedRemainingProgress]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (cooldownSeconds === 0 && retryAfterCooldown.current) {
      retryAfterCooldown.current = false;
  ...
  const celebrateReward = () => {
    ...
    setTimeout(() => {
      deferAllowanceUpdate.current = false;
      setDisplayedRemainingProgress(pendingAllowanceProgress.current);
    }, 720);
