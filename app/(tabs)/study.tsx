...
export default function StudyScreen() {
  ...
  return <ScreenContainer style={styles.screen}><ScrollView contentContainerStyle={[styles.content, isCompact && styles.contentCompact]} showsVerticalScrollIndicator={false}><Text style={[styles.title, isCompact && styles.titleCompact]}>Study Modes</Text><Text style={styles.subtitle}>Choose the right kind of practice for the time you have.</Text><View style={[styles.cards, isCompact && styles.cardsCompact]}>{modes.map((mode) => <Pressable key={mode.key} onPress={() => selectMode(mode.key)} style={({ pressed }) => [pressed && styles.pressed]}><Card style={[styles.modeCard, isCompact && styles.modeCardCompact, mode.tone === "dark" && styles.nightCard]}><View style={[styles.modeIcon, mode.tone === "amber" && styles.amberIcon, mode.tone === "dark" && styles.nightIcon]}><MaterialIcons name={mode.icon} size={22} color={mode.tone === "dark" ? "#FFFFFF" : mode.tone === "amber" ? "#B45309" : colors.primary} /></View><View style={styles.modeText}><View style={styles.modeTitleLine}><Text style={[styles.modeTitle, mode.tone === "dark" && styles.nightText]}>{mode.title}</Text>{mode.key === "quick" ? <Pill label="FAST" tone="amber" /> : null}</View><Text style={[styles.modeDetail, mode.tone === "dark" && styles.nightDetail]}>{mode.detail}</Text></View><MaterialIcons name="chevron-right" size={23} color={mode.tone === "dark" ? "#FFFFFF" : colors.textSecondary} /></Card></Pressable>)}</View><Card style={[styles.tipCard, isCompact && styles.tipCardCompact]}><MaterialIcons name="lightbulb-outline" color="#B45309" size={22} /><View style={styles.tipText}><Text style={styles.tipTitle}>Smart study tip</Text><Text style={styles.tipCopy}>Short, repeated practice is more effective than a single long review.</Text></View></Card></ScrollView></ScreenContainer>;
}

const styles = StyleSheet.create({ screen: { backgroundColor: colors.background }, content: { padding: 20, paddingBottom: 28 }, contentCompact: { padding: 16, paddingBottom: 18 }, title: { color: colors.text, fontSize: 25, fontWeight: "800" }, titleCompact: { fontSize: 22 }, subtitle: { color: colors.textSecondary, fontSize: 14, lineHeight: 20, marginTop: 7 }, cards: { gap: 13, marginTop: 24 }, cardsCompact: { gap: 9, marginTop: 16 }, modeCard: { minHeight: 104, flexDirection: "row", alignItems: "center", padding: 16, gap: 14 }, modeCardCompact: { minHeight: 88, padding: 13, gap: 11 }, nightCard: { backgroundColor: "#111827", borderColor: "#111827" }, modeIcon: { width: 48, height: 48, borderRadius: 16, backgroundColor: "#F0FDF4", alignItems: "center", justifyContent: "center" }, amberIcon: { backgroundColor: colors.accentLight }, nightIcon: { backgroundColor: "rgba(255,255,255,0.14)" }, modeText: { flex: 1 }, modeTitleLine: { flexDirection: "row", alignItems: "center", gap: 8, flexWrap: "wrap" }, modeTitle: { color: colors.text, fontSize: 16, fontWeight: "800" }, nightText: { color: "#FFFFFF" }, modeDetail: { color: colors.textSecondary, fontSize: 12, lineHeight: 17, marginTop: 5 }, nightDetail: { color: "#D1D5DB" }, tipCard: { padding: 16, marginTop: 20, flexDirection: "row", gap: 12, backgroundColor: "#FFFBEB", borderColor: "#FDE68A" }, tipCardCompact: { padding: 13, marginTop: 13 }, tipText: { flex: 1 }, tipTitle: { color: "#92400E", fontSize: 13, fontWeight: "800" }, tipCopy: { color: "#92400E", fontSize: 12, lineHeight: 17, marginTop: 3 }, pressed: { opacity: 0.9, transform: [{ scale: 0.98 }] }, });
